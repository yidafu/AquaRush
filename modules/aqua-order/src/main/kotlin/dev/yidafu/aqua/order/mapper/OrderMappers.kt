...
object OrderModelToDTOMapper : ObjectMappie<OrderModel, OrderDTO>() {
    override fun map(from: OrderModel): OrderDTO {
        ...
        return OrderDTO(
            ...
            deliveryPhotos = from.deliveryPhotos?.let { photos ->
                try {
...
object OrderDTOToGraphQLMapper : ObjectMappie<OrderDTO, Order>() {
    override fun map(from: OrderDTO): Order {
        ...
        return Order(
            ...
            paymentMethod = from.paymentMethod?.let { method ->
                dev.yidafu.aqua.common.graphql.generated.PaymentMethod.valueOf(method.name)
            }?.name,
            paymentTime = from.paymentTime,
            paymentTransactionId = from.paymentTransactionId,
            quantity = from.quantity,
            status = dev.yidafu.aqua.common.graphql.generated.OrderStatus.valueOf(from.status.name),
            updatedAt = from.updatedAt,
...
object OrderMapper : ObjectMappie<OrderModel, Order>() {
    override fun map(from: OrderModel): Order {
        ...
        return Order(
            ...
            deliveryPhotos = from.deliveryPhotos?.let { photos ->
                try {
                    tools.jackson.module.kotlin.jacksonObjectMapper().readValue(photos, Array<String>::class.java).toList()
                } catch (e: Exception) {
                    emptyList()
                }
            },
...
object DeliveryAddressMapper : ObjectMappie<DeliveryAddressModel, DeliveryAddress>() {
  override fun map(from: DeliveryAddressModel) = mapping {
