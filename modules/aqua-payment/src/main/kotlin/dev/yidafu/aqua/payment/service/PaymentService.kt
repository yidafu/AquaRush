...
class PaymentService(
  ...
  fun createWechatJsapiPay(
    ...
    try {
      ...
      if (order.status != dev.yidafu.aqua.common.domain.model.OrderStatus.PENDING_PAYMENT) {
    ...
    try {
      ...
      if (order.status != dev.yidafu.aqua.common.domain.model.OrderStatus.PENDING_PAYMENT) {
        return when (order.status) {
          dev.yidafu.aqua.common.domain.model.OrderStatus.PENDING_DELIVERY -> "SUCCESS"
          dev.yidafu.aqua.common.domain.model.OrderStatus.CANCELLED -> "CLOSED"
          else -> "UNKNOWN"
        }
      }
