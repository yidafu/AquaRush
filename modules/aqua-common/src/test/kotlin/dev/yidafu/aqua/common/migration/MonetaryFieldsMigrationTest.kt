...
class MonetaryFieldsMigrationTest {
  ...
  private val preMigrationProducts = listOf(
  ...
  private val preMigrationOrders = listOf(
  ...
  private val preMigrationPayments = listOf(
  ...
  fun `convert decimal yuan to cents correctly`() {
    ...
    val convertedProducts = preMigrationProducts.map { product ->
  ...
  fun `convert decimal order amounts to cents correctly`() {
    val convertedOrders = preMigrationOrders.map { order ->
      PostMigrationOrder(
        id = order.id,
    ...
    expectedOrders.zip(convertedOrders).forEach { (expected, actual) ->
      assertEquals(expected.id, actual.id)
      assertEquals(expected.orderNumber, actual.orderNumber)
      assertEquals(expected.totalAmount, actual.totalAmount, "Amount conversion failed for ${expected.orderNumber}")
      assertEquals(expected.status, actual.status)
    }
  }
  ...
  fun `convert decimal payment amounts to cents correctly`() {
    val convertedPayments = preMigrationPayments.map { payment ->
  ...
  fun `handle null values correctly during migration`() {
    ...
    val convertedProducts = productsWithNulls.map { product ->
      PostMigrationProduct(
        id = product.id,
        name = product.name,
        price = product.price?.let { MoneyUtils.toCents(it) },
  ...
  fun `verify database migration integrity`() {
    ...
    val migratedProducts = preMigrationProducts.map { product ->
      PostMigrationProduct(
        id = product.id,
        name = product.name,
        price = product.price?.let { MoneyUtils.toCents(it) },
    ...
    val migratedOrders = preMigrationOrders.map { order ->
      PostMigrationOrder(
        id = order.id,
        orderNumber = order.orderNumber,
        totalAmount = order.totalAmount?.let { MoneyUtils.toCents(it) },
    ...
    val migratedPayments = preMigrationPayments.map { payment ->
      PostMigrationPayment(
        id = payment.id,
        amount = payment.amount?.let { MoneyUtils.toCents(it) },
        transactionId = payment.transactionId
    ...
    migratedOrders.forEach { order ->
      assertTrue(
        ...
        "Order amount should be non-negative: ${order.orderNumber}"
      )
    }
    ...
    val totalProductValue = migratedProducts
      .mapNotNull { it.price }
      .sumOf { it }

    val expectedTotalProductValue = preMigrationProducts
      .mapNotNull { it.price }
      .map { MoneyUtils.toCents(it) }
      .sumOf { it }

    assertEquals(
      expectedTotalProductValue, totalProductValue,
  ...
  fun `verify rollback capability`() {
    ...
    val rolledBackProducts = migratedProducts.map { product ->
      PreMigrationProduct(
        id = product.id,
        name = product.name,
    ...
    preMigrationProducts.zip(rolledBackProducts).forEach { (original, rolledBack) ->
      ...
      assertEquals(
        original.price, rolledBack.price,
  ...
  fun `test migration performance with large datasets`() {
    ...
    val largeDataset = (1L..10000L).map { id ->
      PreMigrationProduct(
        id = id,
        name = "Product $id",
    ...
    val convertedDataset = largeDataset.map { product ->
      PostMigrationProduct(
        id = product.id,
        name = product.name,
        price = product.price?.let { MoneyUtils.toCents(it) },
    ...
    val sampleProducts = listOf(1L, 5000L, 9999L).map { id ->
