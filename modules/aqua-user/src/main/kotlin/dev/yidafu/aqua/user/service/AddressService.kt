...
class AddressService(
    ...
    fun updateAddress(addressId: Long, updates: AddressUpdateRequest): AddressModel? {
        ...
        updates.postalCode?.let { existingAddress.postalCode = it }
    ...
    @CacheEvict(value = ["user_addresses", "user_default_address", "address"],
                  key = "#addressId", allEntries = true)
    fun deleteAddress(addressId: Long, userId: Long): Boolean {
     ...
     */
    @CacheEvict(value = ["user_addresses", "user_default_address"], key = "#userId")
    fun setDefaultAddress(addressId: Long, userId: Long): Boolean {
        val address = addressRepository.findById(addressId)
            .orElseThrow { AquaException("地址不存在") }
