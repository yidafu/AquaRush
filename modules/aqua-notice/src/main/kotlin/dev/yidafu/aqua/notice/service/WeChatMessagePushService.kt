...
import dev.yidafu.aqua.notice.domain.model.*
...
class WeChatMessagePushService(
    ...
    fun sendMessage(
        ...
        try {
            val templateId = weChatProperties.templates[messageType.templateId]
                ?: throw IllegalArgumentException("Template ID not found for message type: $messageType")

            val weChatTemplate = WeChatTemplate(
                touser = openId,
                template_id = templateId,
                page = page,
                data = templateData.mapValues { (_, value) ->
                    WeChatTemplateData(value = value)
                }
            )

            val content = objectMapper.writeValueAsString(weChatTemplate)
            val messageHistory = MessageHistory.createPending(
                userId = userId,
            ...
            try {
                val messageHistory = MessageHistory.createFailure(
                    userId = userId,
                    messageType = messageType.templateId,
                    templateId = "",
                    content = "",
    ...
    fun retryFailedMessages(): Int {
        ...
        for (message in failedMessages) {
            try {
                ...
                val originalTemplate = objectMapper.readValue(message.content, WeChatTemplate::class.java)
