...
class AdminNoticeMutationResolver(
    ...
    @Transactional
    fun createNotificationTemplate(@Valid input: CreateNotificationTemplateInput): NotificationTemplate {
        try {
            // 验证输入
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Transactional
    fun broadcastSystemMessage(@Valid input: BroadcastMessageInput): BroadcastResult {
        try {
            // 验证输入
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Transactional
    fun sendTargetedMessage(@Valid input: SendTargetedMessageInput): SendResult {
        try {
            // 验证输入
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Transactional
    fun toggleNotificationTemplate(id: Long, isActive: Boolean): Boolean {
        return try {
            // TODO: 实现从服务激活/停用通知模板
