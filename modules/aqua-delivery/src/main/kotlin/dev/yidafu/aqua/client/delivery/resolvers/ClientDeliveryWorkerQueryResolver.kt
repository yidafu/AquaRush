...
class ClientDeliveryWorkerQueryResolver(
    ...
    fun onlineDeliveryWorkersPublic(): List<DeliveryWorker> {
        return deliveryService.getOnlineWorkers().map { worker ->
            // 返回公开信息，隐藏敏感数据
            DeliveryWorker(
                id = worker.id,
                userId = 0L, // 隐藏用户ID
                wechatOpenId = "", // 隐藏微信OpenID
                name = worker.name,
                phone = "", // 隐藏手机号
                ...
                earning = BigDecimal.ZERO, // 隐藏收入信息
    ...
    fun myDeliveryWorkerProfile(): DeliveryWorker? {
        ...
        return deliveryService.getWorkerById(currentWorkerId)
    }
