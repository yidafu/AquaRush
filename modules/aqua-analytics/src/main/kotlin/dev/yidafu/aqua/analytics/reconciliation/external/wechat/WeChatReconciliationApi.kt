...
package dev.yidafu.aqua.analytics.reconciliation.external.wechat

import dev.yidafu.aqua.analytics.reconciliation.external.config.ReconciliationConfig
import dev.yidafu.aqua.analytics.reconciliation.external.wechat.dto.WeChatRefundRecord
import dev.yidafu.aqua.analytics.reconciliation.external.wechat.dto.WeChatSettlementRecord
import dev.yidafu.aqua.analytics.reconciliation.external.wechat.dto.WeChatTransactionRecord
...
class WeChatReconciliationApi(
    ...
    try {
      ...
      for (i in 1 until lines.size) {
        ...
        if (line.isNotEmpty()) {
          ...
          if (fields.size >= 20) {
            val record =
              WeChatTransactionRecord(
                transactionId = fields[0].trim(),
                outTradeNo = fields[1].trim(),
                transactionType = fields[2].trim(),
                tradeState = fields[3].trim(),
                bankType = fields[4].trim().takeIf { it.isNotEmpty() },
                totalFee = fields[5].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull(),
                feeType = fields[6].trim().takeIf { it.isNotEmpty() },
                cashFee = fields[7].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull(),
                refundFee = fields[8].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull() ?: BigDecimal.ZERO,
                couponFee = fields[9].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull() ?: BigDecimal.ZERO,
                body = fields[10].trim(),
                detail = fields[11].trim().takeIf { it.isNotEmpty() },
                attach = fields[12].trim().takeIf { it.isNotEmpty() },
                deviceInfo = fields[13].trim().takeIf { it.isNotEmpty() },
                goodsTag = fields[14].trim().takeIf { it.isNotEmpty() },
                timeEnd = parseWeChatDateTime(fields[15].trim()),
                tradeCreateTime = parseWeChatDateTime(fields[16].trim()),
                tradePayTime = parseWeChatDateTime(fields[17].trim()),
                settlementTotalFee = fields[18].trim().takeIf { it.isNotEmpty() }?.toLong(),
                promotionInfo = fields[19].trim().takeIf { it.isNotEmpty() },
                amount = fields[20].trim().toLong(),
                openId = if (fields.size > 21) fields[21].trim().takeIf { it.isNotEmpty() } else null,
                subOpenId = if (fields.size > 22) fields[22].trim().takeIf { it.isNotEmpty() } else null,
                tradeBill = if (fields.size > 23) fields[23].trim().takeIf { it.isNotEmpty() } else null,
                goodsDetail = if (fields.size > 24) fields[24].trim().takeIf { it.isNotEmpty() } else null,
                costType = if (fields.size > 25) fields[25].trim().takeIf { it.isNotEmpty() } else null,
                profitSharing = if (fields.size > 26) fields[26].trim().takeIf { it.isNotEmpty() } else null,
                returnCode = "SUCCESS",
                returnMsg = "获取成功",
                errorCode = null,
                errorCodeDes = null,
   ...
   * 解析退款记录响应
   */
  private fun parseRefundResponse(
    response: String,
    date: LocalDate,
  ): List<WeChatRefundRecord> {
    try {
      // 类似交易记录的解析逻辑
      val lines = response.split("\n")
          ...
          if (fields.size >= 15) {
            val record =
              WeChatRefundRecord(
                ...
                refundChannel = fields[4].trim().takeIf { it.isNotEmpty() },
                refundFee = fields[5].trim().toBigDecimal(),
                settlementRefundFee = fields[6].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull(),
                totalFee = fields[7].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull(),
                settlementTotalFee = fields[8].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull(),
                couponType = fields[9].trim().takeIf { it.isNotEmpty() },
                couponRefundFee = fields[10].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull() ?: BigDecimal.ZERO,
                couponFee = fields[11].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull() ?: BigDecimal.ZERO,
                amount = fields[12].trim().toBigDecimal(),
                refundRecvAccount = fields[13].trim().takeIf { it.isNotEmpty() },
                refundAccount = fields[14].trim().takeIf { it.isNotEmpty() },
                refundRequestSource = if (fields.size > 15) fields[15].trim().takeIf { it.isNotEmpty() } else null,
                refundSuccessTime =
                  if (fields.size > 16) {
                    parseWeChatDateTime(
                      fields[16].trim(),
   ...
   * 解析结算记录响应
   */
  private fun parseSettlementResponse(
    response: String,
    date: LocalDate,
  ): List<WeChatSettlementRecord> {
    try {
      // 类似交易记录的解析逻辑
      val lines = response.split("\n")
          ...
          if (fields.size >= 25) {
            val record =
              WeChatSettlementRecord(
                ...
                mchId = fields[5].trim(),
                subMchId = fields[6].trim().takeIf { it.isNotEmpty() },
                deviceInfo = fields[7].trim().takeIf { it.isNotEmpty() },
                billType = fields[8].trim(),
                tradeType = fields[9].trim(),
                tradeState = fields[10].trim(),
                bankType = fields[11].trim().takeIf { it.isNotEmpty() },
                feeType = fields[12].trim().takeIf { it.isNotEmpty() },
                totalFee = fields[13].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull(),
                settlementTotalFee = fields[14].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull(),
                couponFee = fields[15].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull() ?: BigDecimal.ZERO,
                couponRefundFee = fields[16].trim().takeIf { it.isNotEmpty() }?.toBigDecimalOrNull() ?: BigDecimal.ZERO,
                ...
                goodsTag = fields[24].trim().takeIf { it.isNotEmpty() },
                ...
                promotionInfo = if (fields.size > 29) fields[29].trim().takeIf { it.isNotEmpty() } else null,
