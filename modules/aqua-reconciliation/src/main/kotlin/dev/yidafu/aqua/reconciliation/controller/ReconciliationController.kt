...
class ReconciliationController(
    ...
    return try {
      val tasks =
        if (startDate != null && endDate != null) {
          ...
          reconciliationService.getReconciliationTasks(pageable).content
        }
  ...
  fun getReconciliationTask(
    @PathVariable taskId: String,
  ): ApiResponse<ReconciliationTask> {
    return try {
      val task =
        reconciliationService.getReconciliationTask(taskId)
  ...
  fun resolveDiscrepancy(
    ...
    return try {
      val currentUser = SecurityContextHolder.getContext().authentication?.name ?: "unknown"
